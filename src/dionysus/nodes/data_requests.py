...
from dionysus.conf_default.globals import ROOT_URL, CHALLENGE_SUB_URL, CHALLENGE_VIDEOS_SUB_URL
...
def request_json_data(url: str, browser_params: BrowserParams) -> Dict[str, Any]:
    ...
    if json_data['status_code'] != 0:
        ...
        logger.debug(f'The request to path {url} with the following headers:\n'
...
def form_hashtag_videos_urls(hashtag_id: int, ms_token: str) -> str:
...
def request_hashtag_videos_json_data(
    hashtag_videos_urls: List[str], 
    browser_params: BrowserParams) -> List[Dict[str, Any]]:
    ...
    while has_more:
        hashtag_videos_json_data = request_json_data(
            url=hashtag_videos_urls[n_completed],
        ...
        if has_more:
            logger.debug(f'TikTok indicates more videos are available '
            ...
            for video_json_data in hashtag_videos_json_data['itemList']:
                ...
                video_json_data.update({'now': hashtag_videos_json_data['extra']['now']})
                list_video_json_data.append(video_json_data)
        else:
            logger.info('Tiktok indicates no more videos exist '
                        f'after the {n_completed}th request')
            break
